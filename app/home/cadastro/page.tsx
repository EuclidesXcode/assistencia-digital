  ...
  return (
    <div className="space-y-6 min-w-0">
      <header>
        ...
        <p className="text-sm text-slate-500 mt-1">Um Código NF, EAN e Modelo Referência pode estar vinculado a vários Modelos Fabricante e exige cadastro de Acessórios Referentes.</p>
      </header>
      ...
      <section className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <p className="text-sm text-slate-500 mb-4">Clique em um modelo para selecioná-lo e vincular peças estéticas e funcionalidades.</p>

        <div className="flex flex-col sm:flex-row gap-3 items-center">
          <input name="modeloFabricante" value={form.modeloFabricante} onChange={onChange} placeholder="EX.: 50UT8050PSA.BWZQLL" className={`flex-1 rounded-md px-3 py-2 text-sm ${errors.modelos ? 'border-red-500' : 'border-slate-200 border'}`} />
        ...
        <div className="mt-4 overflow-auto">
          <table className="w-full table-fixed text-left text-sm">
            ...
            <tbody>
              {modelos.length === 0 ? (
                <tr>
                  <td colSpan={3} className="py-6 text-center text-slate-400">Nenhum modelo fabricante incluído.</td>
                </tr>
              ) : (
                modelos.map((m, idx) => (
      ...
      <section className="bg-white rounded-lg border border-dashed border-slate-200 p-6">
        ...
        <p className="text-sm text-slate-500 mb-4">Lista unificada de embalagem, acessórios, peças estéticas e funcionalidades associados a este Código NF / EAN / Modelo Referência.</p>
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className={`flex-1 rounded-md px-3 py-2 text-sm ${errors.itens ? 'border-red-500' : 'border-slate-200 border'}`}
            />
          ...
          <div className="overflow-auto">
            <table className="w-full table-fixed text-left text-sm">
              ...
              <tbody>
                {itens.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-6 text-center text-slate-400">Nenhuma peça vinculada ainda.</td>
                  </tr>
                ) : (
                  itens.map((it, i) => (
      ...
      <footer className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <p className="text-xs text-slate-500">Campos obrigatórios: Código NF, EAN, Modelo Referência, pelo menos 1 Modelo Fabricante e ao menos 1 Acessório Referente.</p>
