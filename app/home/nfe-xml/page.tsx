  ...
  return (
    <div className="space-y-6 min-w-0">
        ...
        <div className="flex flex-col lg:flex-row lg:flex-nowrap items-stretch lg:items-center gap-2 w-full lg:w-auto">
          ...
          <div className="text-sm flex items-center gap-2 w-full lg:w-auto justify-end">
            <div className="hidden lg:flex items-center gap-3">
              <div>
                <div className="text-xs text-slate-600">USUÁRIO</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200 text-center">
          <div className="text-xs text-slate-600">DIVERGENTES</div>
          <div className="text-2xl font-bold">{stats.divergentes}</div>
        </div>
        <div className="bg-white p-4 rounded-lg border border-slate-200 text-center">
          <div className="text-xs text-slate-600">CONFERIDAS</div>
      ...
      <section className="bg-white rounded-lg border border-slate-200 p-4">
        ...
        <div className="md:hidden space-y-3">
          {filtered.length === 0 ? (
            <div className="p-6 text-center text-slate-600">Nenhuma nota carregada.</div>
          ) : (
            filtered.map((n, i) => (
              <div key={n.chave} className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'} border border-slate-100 rounded-lg p-3`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-xs text-slate-600">CHAVE</div>
                    <div className="font-mono text-sm truncate max-w-full" style={{ color: '#6b7280' }}>{n.chave}</div>
                    <div className="text-xs text-slate-600 mt-2">N.º NF</div>
                    <div className="text-sm" style={{ color: '#6b7280' }}>{n.numero}</div>
                  </div>
        ...
        <div className="hidden md:block overflow-auto">
          <table className="w-full text-sm table-fixed">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-600">Nenhuma nota carregada.</td>
                </tr>
              ) : (
                filtered.map((n, i) => (
                  <tr key={n.chave} className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
                    <td className="p-3 font-mono break-words max-w-[14rem] whitespace-normal text-slate-700">{n.chave}</td>
                    <td className="p-3 text-slate-700">{n.numero}</td>
                    <td className="p-3 hidden lg:table-cell text-slate-700">{n.emissao}</td>
                    <td className="p-3 hidden lg:table-cell text-slate-700">{n.itens}</td>
