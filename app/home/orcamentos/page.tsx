      ...
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        <section className="bg-white p-0 md:p-8 rounded-[2rem] shadow-xl border border-slate-100 relative overflow-hidden">
          ...
          <div className="lg:hidden space-y-4 px-6 md:px-0">
            ...
            {filteredRegistros.length === 0 ? (
              <div className="p-8 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-400">Nenhum orçamento encontrado.</div>
            ) : filteredRegistros.map((r) => (
              <div key={r.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-100 relative group active:scale-[0.98] transition-all">
                ...
                <div className="flex flex-col gap-4">
                  <div>
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-yellow-100 text-yellow-700 border border-yellow-200 mb-2">Pendente</span>
                  ...
                  <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm bg-white p-3 rounded-xl border border-slate-100">
                    <div><span className="text-[10px] uppercase font-bold text-slate-400 block mb-0.5">Nota Fiscal</span><span className="font-semibold text-slate-700">{r.nf}</span></div>
                    <div><span className="text-[10px] uppercase font-bold text-slate-400 block mb-0.5">Analista</span><div className="flex items-center gap-1.5"><User size={12} className="text-blue-500" /><span className="font-medium text-slate-700 truncate">{r.analisadoPor}</span></div></div>
                    <div className="col-span-2 pt-2 border-t border-slate-50 flex justify-between items-center"><span className="text-xs text-slate-400 flex items-center gap-1"><Calendar size={10} /> {r.data}</span><span className="text-xs font-mono text-slate-500 flex items-center gap-1"><Barcode size={10} /> {r.ean}</span></div>
                  </div>
                </div>
              </div>
            ))}
